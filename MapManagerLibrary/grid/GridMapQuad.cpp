...
#include "GridMapQuad.h" 
#include <fstream.h>

template GridMapQuad<double>;
template GridMapQuad<int>;
template GridMapQuad<float>;


//default constructor
template <class T>
GridMapQuad<T>::GridMapQuad():Grid3DQuad<T>()
{
...
//class constructor
template <class T>
GridMapQuad<T>::GridMapQuad(int blocksize, int radius, T Unknown):Grid3DQuad<T>(blocksize, radius, Unknown, 1)
{	 
	//reportfile.open("c:\\temp\\gmapReport.txt");
	//GET_FILE_LOG
	LOGGING_OFF
...
template <class T>
GridMapQuad<T>::GridMapQuad(GridBlockQuad<T>* newGridBlock, long *dim, long blocksize,T Unknown):Grid3DQuad<T>(blocksize,1,Unknown,1)
{
...
template <class T>
void GridMapQuad<T>::copy(Grid3DQuad<T>* mapToCopy,int reduceFactor,int valueToSelect)
{
	T maxVal=-10000;
	T minVal= 10000;
	T newVal = 0;
	...
	{					 //used to be 4
		...
		for(int y = North - 1; y> South -1; y-=2)
		{
			...
			if(maxVal != 0 && valueToSelect == LARGEST_VALUE_QUAD) //no point updating the map with a value that it has by default
			{			   //this will make the map smaller if the original has many empty cells
				//near the border
				updateGridRef(maxVal,x/2,y/2);
							
			}
			if(minVal != 0 && valueToSelect == SMALLEST_VALUE_QUAD) //no point updating the map with a value that it has by default
			{			   //this will make the map smaller if the original has many empty cells
				//near the border
...
template <class T>
void GridMapQuad<T>::reduceDimension(int reduceFactor, int valueToSelect)
{
	//create a new map, copy our map into it, then copy it back while shrinking it
	GridMapQuad<T> *tempMap = new GridMapQuad<T>(myMap, dimensions, blockSize, unknown);
...
*/
template <class T>
void GridMapQuad<T>::boxBlur(int kernelSize, double boxVal)
{
	T tempProb=0;
	int x,y;	
	int k;
	
	GridMapQuad<T>* oldMap;
	
	//copy the map to be blurred into another GridMapQuad, without duplicating the data
	oldMap = new GridMapQuad<T>(myMap,dimensions,blockSize, unknown);
...
template <class T>
void GridMapQuad<T>::gaussBlur(int kernelSize)
{
	long Xmax, Ymax, Xmin, Ymin; //store the dimensions of the main map 		
	double sum=0;	 
	...
	//create a new map that uses the same mapblocks as this GridMapQuad
...
//------------------------------------------------------------------------------------
/*
//convert a .wld file into a GridMapQuad
template <class T>
bool GridMapQuad<T>::importPointMap(char* fileName, double value, long squareSize)
{
	pointMap *pMap = 0;
...
template <class T>
bool GridMapQuad<T>::addLine(long x1, long y1, long x2, long y2, T value, long squareSize, bool doubleLine)
{
	long temp = 0;
	...
	{	//if the distance between the two y's is greater than or equal to the distance
		...
		{		
			...
			for(y=y1; y < y2 + (squareSize - (y2 % squareSize)) ; y += squareSize)
			{
				...
				if(doubleLine)
				{
					updateGridRef(value,(x+positionDiff)/squareSize, y/squareSize); 
				}
			}			
	...
	{	
		...
		if(!noSlope)
		{
			...
			for(x=x1; x < x2 + (squareSize - x2 % squareSize) ; x += squareSize)
			{
				...
				if(doubleLine)
				{
					updateGridRef(value, x/squareSize, (y + positionDiff)/squareSize);				
				}
			}
		...
		else
		{
			...
			for(x=x1; x < x2 + (squareSize - x2 % squareSize) ; x += squareSize)
			{
				...
				if(doubleLine)
				{
					updateGridRef(value, x/squareSize, (y+positionDiff)/squareSize); //set the occupied value of the cell to 1	
				}
			}
		}
	}	
...
//of the two maps
template <class T>
double GridMapQuad<T>::correlateMap(GridMapQuad<T>* mapToCompare)
{
	double result=0;
...
template <class T>
double GridMapQuad<T>::scoreMap(GridMapQuad<T>* mapToCompare, bool justCompareOccAreas)
{
  double score = 0;
...
//this function uses Carnegie Mellon's MATCH method to compare two maps.
template <class T>
double GridMapQuad<T>::scoreMap(GridMapQuad<T>* mapToCompare)
{
  double score = 0;
...
template <class T>
bool GridMapQuad<T>::growOccArea(long radius, T lowerBound, T upperBound, long squaresize)
{
  ...
  long oldMapDimensions[4]; 
	...
	oldMapDimensions[NORTH%4] = dimensions[NORTH%4];
	oldMapDimensions[SOUTH%4] = dimensions[SOUTH%4];
	oldMapDimensions[EAST%4]  = dimensions[EAST%4];
	oldMapDimensions[WEST%4]  = dimensions[WEST%4];

	//first, create a new GridMapQuad, copy our map into it
	GridMapQuad<T> *oldMap;
	oldMap = new GridMapQuad<T>(blockSize, 1, unknown);
	oldMap->copy(this);
	
	//now we start processing, reading in values from the oldMap, and inserting them	
	//into our new map
	for(long x = oldMapDimensions[WEST%4] * squaresize + squaresize/2; 
	x <= oldMapDimensions[EAST%4]*squaresize + squaresize/2; x+=squaresize)
	{
	for(long y = oldMapDimensions[NORTH%4] * squaresize + squaresize/2
		; y >= oldMapDimensions[SOUTH%4]*squaresize + squaresize/2; y-=squaresize)
	{
...
template <class T>
double GridMapQuad<T>::getGridRefFromTopLeftView(long inputX, long inputY)
{
	if(!topLeftSet)
	{
...
/*
template <class T>
double GridMapQuad<T>::getGridRefFromView(long inputX, long inputY)
{
...
*/
template <class T>
double GridMapQuad<T>::getGridRefFromView(long inputX, long inputY,long viewHeight, long viewWidth)
{	/*
...
template <class T>
double GridMapQuad<T>::getGridRefFromTopLeftView(long inputX, long inputY,long viewHeight, long viewWidth)
{
	double actualWidth = getMapWidth();
	double actualHeight = getMapHeight();
			...
			,long mapWidth, long mapHeight)
{
	...
	for(long x = (long)SosUtil::roundDown(leftX); x<SosUtil::roundUp(rightX); x++)
	{
		for(long y = (long)SosUtil::roundDown(topY); y >= SosUtil::roundDown(bottomY); y--)
		{
			...
			if(SosUtil::between(rightX,(double)x,(x+1)))
				bigX = rightX;
			else
				bigX = x+1;
...
template <class T>
void GridMapQuad<T>:: setTopLeftPos(long x, long y)
{
	topLeftX = x;
	topLeftY = y;
	topLeftSet = true;
...
template <class T>
bool GridMapQuad<T>:: addLineFromView(long x1, long y1, long x2, long y2, T value, long squareSize, 
